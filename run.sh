...
set -e    # Settigng -e will cause the script to exit immediately if a command fails

# this line will get the current directory of the script run.sh
...
function test:ci {
    INSTALLED_PKG_DIR="$(python -c 'import aprofs; print(aprofs.__path__[0])')"
...
function run-tests {
    ...
    python -m pytest ${@:-"$THIS_DIR/tests/"} \
        --cov "${COVERAGE_DIR:-$THIS_DIR/src/aprofs}" \
        --cov-report html \
        --cov-report term \
        --cov-report xml \
        --junit-xml "$THIS_DIR/test-reports/report.xml" \
        --cov-fail-under 10 || ((PYTEST_EXIT_STATUS+=$?))
    # moving the files into subdirectories to keep the root clean
...
function clean:all {
    rm -rf dist build coverage.xml test-reports site test-env #.git
    find . \
      -type d \
      \( \
        -name "*cache*" \
...
function init-project {
    if [ -d ".git" ]; then
        ...
        git add --all
        git branch -M main
        git commit -m "feature: generated sample project"
